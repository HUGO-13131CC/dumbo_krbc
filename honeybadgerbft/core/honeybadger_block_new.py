...
def honeybadger_block(pid, N, f, PK, SK, propose, acs_put_in, acs_get_out, tpke_bcast, tpke_recv, logger=None):
    ...
    ciphertext = tpke.encrypt(key, propose)     # 对称加密（密文）交易（ciphertext应为队列）
    tkey = PK.encrypt(key)      # 对称加密的密钥做了门限公钥加密（密文）密钥


    #print("node %d starts to make block" % pid)

    import pickle
    to_acs = pickle.dumps((serialize_UVW(*tkey), ciphertext))       # 密钥和密文均作序列化
    acs_put_in(to_acs)
