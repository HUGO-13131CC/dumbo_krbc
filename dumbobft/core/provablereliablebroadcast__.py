...
def provablereliablebroadcast(sid, pid, N, f,  PK2s, SK2, leader, input, receive, send, idx, num_of_seg, logger=None):
        ...
        if msg[0] == 'VAL' and fromLeader is None:
            ...
            if roothash in stripes and stripes[roothash][sender] is not None \
               or sender in echoSenders:
                print(f"Redundant ECHO {sender in echoSenders}")
                continue
            try:
            ...
            if sender in ready[roothash] or sender in readySenders:
                print(f"Redundant READY {sender in ready[roothash]}, {sender in readySenders}")
                continue
            # try:
            #     digest = hash((sid, roothash))
