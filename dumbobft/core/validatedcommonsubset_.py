...
def validatedcommonsubset(sid, pid, N, f, PK, SK, PK1, SK1, PK2s, SK2, input, decide, receive, send, predicate=lambda i, v: True, logger=None):
    ...
    # Using a pool to store predicate coroutines and release the all at the end
    predicate_pool = []
    ...
    #             if len(m) == N:
    #                 for i in range(N):
    #                     pre_pt = gevent.spawn(predicate_pt, i)
    #                     # pre_pt.start()
    #                     predicate_pool.append(pre_pt)
    #
    #         # TODO: gevent 事件改写
    #         while counter.get() < N - f:
    #             time.sleep(0)
    #             gevent.sleep(0)
    #             continue
    #         for pt in predicate_pool:
    #             pt.kill()
    #         return True
    #
    ...
    while True:
        j, vj = value_recv.get()
        try:
            pre_pt = gevent.spawn(predicate, j, vj)
            predicate_pool.append(pre_pt)
            assert pre_pt.get()
            valueSenders.add(j)
            values[j] = vj
